...
void recoveryHandle(void)
{
	...
	switch (mvBoardIdGet())
	{
		...
		case DB_88F6535_BP_ID:
			setenv("bootcmd","setenv bootargs $(console) ubi.mtd=2 root=ubi0:rootfsU rootfstype=ubifs $(mvNetConfig) $(mvPhoneConfig; nand read.e $(loadaddr) 0x200000 0x300000; bootm $(loadaddr);");
