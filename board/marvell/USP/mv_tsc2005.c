...
{
	u32 temp_reg;
	u32 mpp_control_reg = MPP_CONTROL_REG0 + (mpp >> 3) *4;
...
static
int ts_report_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	  ulong start;
	  ulong timeout;
	  int x,y;
...
static
int ts_test_cmd(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	ulong start;
	int pass=0;
	printf("Please touch the upper-left corner\n");
	start = get_timer(0);
	do{
		  if(tsc2005_touched()){
			  touched = 1;
			  ...
			  if((x < UPPER_RIGHT_X_RANGE) && (y < UPPER_RIGHT_Y_RANGE)){
				  pass++;
			  }else{
		  ...
		  udelay(DELAYS);	//delay 20msecs
	}while(get_timer(start) < TIMEOUT_3_SECONDS);
	...
	printf("Please touch the lower-right corner\n");
	start = get_timer(0);
	do{
		  if(tsc2005_touched()){
			  touched = 1;
			  ...
			  if((x > LOWER_LEFT_X_RANGE) && (y > LOWER_LEFT_Y_RANGE)){
				  pass++;
			  }else{
				  printf("Not in the range\n");
			  }
